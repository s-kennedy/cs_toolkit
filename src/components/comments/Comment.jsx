...
const Comment = props => {
  ...
  const date = new Date(props.comment.timestamp).toLocaleString('en-US', { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric' });
  return(
      ...
      <Grid item xs={12}>
        <div className={props.classes.container}>
          <div className="author" className={props.classes.author}>
            { user.photoURL ?
              <img src={user.photoURL} className={props.classes.img} alt={user.displayName} title={user.displayName} /> :
